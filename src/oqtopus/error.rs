...
/// Maps a failed `oqtopus_cloud` call onto [`QrmiError`]. `resource_kind`
...
/// implements `std::error::Error` via `oqtopus_cloud`'s own generated
...
{
    if let oqtopus_cloud::apis::Error::ResponseError(ref content) = err {
        ...
        match content.status {
            ...
            StatusCode::NOT_FOUND => {
                return match resource_kind {
                    ResourceKind::Device => QrmiError::ResourceNotFound(body),
...
/// Wraps an arbitrary error as [`QrmiError::Other`], with a short
/// description of what was being attempted, for errors that don't come
/// from `oqtopus_cloud` and so don't warrant [`classify`] -- e.g. a raw
/// `reqwest::Error` from a call that bypasses the generated client.
pub(crate) fn other(
    context: impl Into<String>,
    source: impl std::error::Error + Send + Sync + 'static,
) -> QrmiError {
    QrmiError::Other(anyhow::Error::new(source).context(context.into()))
}
